...
public class DbPool {
	...
	public DbPool( String url, String login, String password, Integer min, Integer max ) throws RuntimeException {
		System.out.println("[SMF] DbPool initializing "+url+" as "+login+" (Size "+min+"-"+max+")");
		
		// Check parameters
		if (db_url == null || db_url.matches("^jdbc\\:")) {
			throw new RuntimeException("Invalid JDBC URL "+db_url);
		}
		if (db_min < 1) {
